...
void Main(in FVertexInput In, out FVertexOutput Out)
{
    ...
    u_view_without_translate[0][3] = 0;
    u_view_without_translate[1][3] = 0;
    u_view_without_translate[2][3] = 0;
    
    float3 camera_based_pos = mul(u_view_without_translate, after_rotation).xyz;
    float3 normalized_pos = normalize(camera_based_pos);

    float4x4 u_projection_without_clipping = u_projection;
    u_projection_without_clipping[2][2] = -1.0;
    u_projection_without_clipping[2][3] = .0;

    // 判断是否是orhto
    if (abs(u_projection[3][3]) < 1e-4) {
        Out.Position = mul(u_projection_without_clipping, float4(normalized_pos, 1.0));
    } else {
