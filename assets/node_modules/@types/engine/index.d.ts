...
declare module 'lib/utils/promise' {
	 type ResolveTransfer<T> = (value?: T | PromiseLike<T>) => void; type RejectTransfer = (reason?: any) => void; type Callback<PromiseT, ThenT> = ((value: PromiseT) => ThenT | PromiseLike<ThenT>) | undefined | null; type Executor<T> = (resolve: ResolveTransfer<T>, reject: RejectTransfer) => void; enum State {
	    Pending = 0,
	    Resolved = 1,
	    Rejected = 2
	}
...
declare module 'engine/error' {
	...
	export enum ErrorCode {
	    ...
	    GetFile_NotLoaded = 512,
	    GetFile_FileIDNotRegistered = 513,
	...
	export class File_MaximumSizeExceededError extends EngineError {
	    constructor(detail: {
	...
	export class GetFile_FileIDNotRegisteredError extends EngineError {
	    constructor(detail: {
	        fileID: string;
	    });
	}
...
declare module 'engine/assets/cache/fs' {
	import LitePromise from 'lib/utils/promise'; const mkdir: (dirPath: string) => LitePromise<void>; const rmdir: (dirPath: string) => LitePromise<void>; const saveFile: (src: string, dist: string) => LitePromise<void>; const writeFile: (data: string | ArrayBuffer, path: string) => LitePromise<void>; const unlink: (path: string) => LitePromise<void>;
	export { mkdir, rmdir, saveFile, writeFile, unlink };
...
declare module 'engine/assets/cache' {
	...
	interface Cache {
	    ...
	    lastLoadTime: number;
	    lastUseTime: number;
	} type compareFunction = (a: Cache, b: Cache) => number; class CacheManager {
	    /**
	     *
	     *
	     * @type {number}
...
declare module 'engine/assets/lib/priority-download/download' {
	...
	export const _download: (url: string, savePath?: string) => {
...
declare module 'engine/assets/classes/baseResource' {
	 type BaseLoadContext = import('engine/assets/load-contexts/load-context-definition').BaseLoadContext;
...
declare module 'engine/math/utils' {
	import { Immutable } from 'engine/type';
...
declare module 'engine/math/types' {
	import { DeepImmutable } from 'engine/type'; type Vector2 = import('engine/math/vector2').default; type Vector3 = import('engine/math/vector3').default; type Vector4 = import('engine/math/vector4').default; type Quaternion = import('engine/math/quaternion').default; type Matrix3 = import('engine/math/matrix3').default; type Matrix4 = import('engine/math/matrix4').default; type BoundBall = import('engine/math/boundBall').default; type BoundBox = import('engine/math/boundBox').default;
...
declare module 'engine/render/uniformBlockValue' {
	export default class UniformBlockValue {
	    ...
	    static composeUniformBuffer(ub: UniformBlockValue, kvArray: Array<{
	        key: string;
	        value: ArrayLike<number>;
	    }>, dst?: Float32Array): Float32Array;
	    static createShaderUniformDescByConfig(uniforms: Array<{
	        name: string;
	        type: string;
	        array_count?: number;
...
declare module 'engine/assets/resource' {
	import type { DeepImmutable } from 'engine/type';
...
declare module 'engine/materials/buildinShaderDesc' {
	...
	export const MAX_ADDITIONAL_LIGHT = 4;
	export const MAX_CULL_LIGHT_NUM = 4;
	...
	export function getImageDescByLightMode(mode: PipelineTypeString): Array<{
	    name: string;
	    type: wgfx.EnumImageType;
	}>;
...
declare module 'engine/game/transform3D' {
	...
	export default class Transform3D extends TransformBase {
	     ...
	     * @memberof Transform3D
	     */
	    constructor(entity: Entity3D,rawNativeValue?: Float32Array,raw?: Float32Array,offset?: number);
	    /**
...
declare module 'engine/render/buildinVertexLayout' {
	...
	export class BuildInVertexLayoutFactory {
...
declare module 'engine/game/3D/animator/AnimatorBlendTree' {
	import { Nullable } from 'engine/type';
	...
	interface IBlend2D {
	    ...
	    parameterX: Nullable<string>;
	    parameterY: Nullable<string>;
	    positions: Array<[number, number]>;
	}
	interface IBlendDirect {
	    type: BlendTreeType.Direct;
	    parameters: string[];
	}
	export type AnimatorBlendInfo = IBlend1D | IBlend2D | IBlendDirect;
	export interface AnimatorBlendTreeChild {
	    motion: AnimatorBlendTree | Readonly<AnimationClip>;
	...
	export default class AnimatorBlendTree {
	    name: string;
	    static BlendTreeType: typeof BlendTreeType;
	    get children(): Readonly<AnimatorBlendTreeChild[]>;
	    ...
	    get blendType(): BlendTreeType;
	    set blendType(blendType: BlendTreeType);
	    get blendInfo(): AnimatorBlendInfo;
	    set blendInfo(info: AnimatorBlendInfo);
...
declare module 'engine/game/3D/animator/AnimatorCondition' {
	import { Nullable } from 'engine/type';
	...
	export default class AnimatorCondition {
	    static AnimatorConditionMode: typeof AnimatorConditionMode;
	    parameter: Nullable<string>;
...
declare module 'engine/game/3D/animator/AnimatorStateTransition' {
	import AnimatorCondition, { AnimatorConditionMode } from 'engine/game/3D/animator/AnimatorCondition';
	import AnimatorState from 'engine/game/3D/animator/AnimatorState';
	import { Nullable } from 'engine/type';
	...
	export class AnimatorStateTransition {
	    ...
	    static TransitionInterruptionSource: typeof TransitionInterruptionSource;
	    duration: number;
	    /**
...
declare module 'engine/game/3D/animator/AnimatorState' {
	import { Nullable, DeepImmutable } from 'engine/type';
	import AnimationClip from 'engine/assets/classes/animationClip';
	import AnimatorBlendTree from 'engine/game/3D/animator/AnimatorBlendTree';
	import { AnimatorStateTransition } from 'engine/game/3D/animator/AnimatorStateTransition';
...
declare module 'engine/game/3D/animator/AnimatorStateMachine' {
...
declare module 'engine/assets/classes/avatarMask' {
	import BaseResource from 'engine/assets/classes/baseResource';
	import { BaseLoadContext } from 'engine/assets/load-contexts/load-context-definition';
	import { AvatarMaskAsset } from 'engine/assets/resource';
	/**
	 * 模型骨骼资源。
	 * @export
	 * @class Avatar
...
declare module 'engine/game/3D/animator/AnimatorControllerLayer' {
	import { AnimatorStateMachine } from 'engine/game/3D/animator/AnimatorStateMachine';
	import { Nullable } from 'engine/type';
	import AvatarMask from 'engine/assets/classes/avatarMask';
	export enum AnimatorLayerBlendingMode {
	    Override = 0,
	    Additive = 1
	}
	export default class AnimatorControllerLayer {
	    name: string;
...
declare module 'engine/game/3D/animator/AnimatorControllerParameter' {
	...
	export default class AnimatorControllerParameter {
	    name: string;
	    static AnimatorControllerParameterType: typeof AnimatorControllerParameterType;
...
declare module 'engine/game/3D/animator/Instantiate' {
	import { BlendTreeType } from 'engine/game/3D/animator/AnimatorBlendTree';
	import AnimatorController from 'engine/game/3D/animator/AnimatorController';
	import { AnimatorLayerBlendingMode } from 'engine/game/3D/animator/AnimatorControllerLayer';
	import { AnimatorControllerParameterType } from 'engine/game/3D/animator/AnimatorControllerParameter';
	import { TransitionInterruptionSource } from 'engine/game/3D/animator/AnimatorStateTransition';
	import { DeepImmutable, Nullable } from 'engine/type';
	import { AnimatorConditionMode } from 'engine/game/3D/animator/AnimatorCondition';
	interface IAnimatorControllerDescription {
	    ...
	    transitions: Array<{
	        ...
	        conditions: Array<{
	            name: string;
	            operator: AnimatorConditionMode;
	            value: number;
	        }>;
	        fixedDuration: boolean;
	        duration: number;
	    ...
	    blendTrees: Array<{
	        ...
	        blend: {
	            ...
	            parameterX: number;
	            parameterY: number;
	            type: BlendTreeType.FreeformDirectional2D | BlendTreeType.FreeformCartesian2D | BlendTreeType.SimpleDirectional2D;
	        };
	    }>;
	    parameters: Array<{
	        name: string;
	...
	export default function Instantiate(data: AnimatorControllerDescription, result?: AnimatorController): AnimatorController;
	export {};

}
declare module 'engine/game/3D/animator/AnimatorController' {
	import { Emitter } from 'engine/misc/emitter';
	import AnimatorControllerLayer from 'engine/game/3D/animator/AnimatorControllerLayer';
	import AnimatorControllerParameter, { AnimatorControllerParameterType } from 'engine/game/3D/animator/AnimatorControllerParameter';
	import Instantiate from 'engine/game/3D/animator/Instantiate';
	export enum LayerChangeEvent {
	    Add = 0,
	    Remove = 1
	}
	export enum ParamenterChangeEvent {
	    Add = 0,
	    Remove = 1
	}
	export default class AnimatorController {
	    name: string;
	    static CreateFromAsset: typeof Instantiate;
	    static LayerChangeEvent: typeof LayerChangeEvent;
	    static ParamenterChangeEvent: typeof ParamenterChangeEvent;
	    get layers(): Readonly<AnimatorControllerLayer[]>;
	    get parameters(): Readonly<AnimatorControllerParameter[]>;
	    get paramenterChangeEvent(): Emitter<AnimatorControllerParameter, ParamenterChangeEvent>;
	    constructor(name?: string);
	    ...
	    addParameter(name: string, type: AnimatorControllerParameterType): void;
...
declare module 'engine/assets/classes/animatorController' {
	import { default as RuntimeAnimatorController } from 'engine/game/3D/animator/AnimatorController';
	import { BaseLoadContext } from 'engine/assets/load-contexts/load-context-definition';
	import BaseResource from 'engine/assets/classes/baseResource';
	import { AnimatorControllerDescription } from 'engine/game/3D/animator/Instantiate';
	/**
	 * 动画状态机资源。
	 * @export
...
declare module 'engine/game/3D/animator/AvatarBinding' {
	...
	export class AvatarUtils {
	    ...
	    static GetNodeByPath(root: Transform3D, path: string): Nullable<Transform3D>;
	}
	export class AvatarBinding {
	    get hasShadowTree(): boolean;
	    constructor(avatar: Nullable<Avatar>,rootNode: Transform3D);
	    extractShadowBone(path: string, name?: string): void;
	    getBoneByPath(path: string): Nullable<wxGA.Entity3D>;
	    getNodeByPath(root: Transform3D, path: string): Nullable<Transform3D>;
	    dispose(): void;
	}
	export {};

}
declare module 'engine/game/3D/animator/ControllerBinding' {
	import AnimationClip from 'engine/assets/classes/animationClip';
	import Transform3D from 'engine/game/transform3D';
	import { SmartArray } from 'engine/misc/array';
	import { DeepImmutable, Nullable } from 'engine/type';
	import AnimatorBlendTree from 'engine/game/3D/animator/AnimatorBlendTree';
	import { AvatarBinding } from 'engine/game/3D/animator/AvatarBinding';
	import { MetaAnimationClipBinding, MetaAnimationClipModel, MetaAnimatorControllerModel } from 'engine/game/3D/animator/MetaAnimation';
	import AnimatorContorller from 'engine/game/3D/animator/AnimatorController';
	import { Emitter } from 'engine/misc/emitter';
	import Avatar from 'engine/assets/classes/avatar';
	export class ControllerBinding {
	    readonly rebindEvent: Emitter<void>;
	    get controller(): Nullable<AnimatorContorller>;
	    set controller(controller: Nullable<AnimatorContorller>);
	    get avatar(): Nullable<Avatar>;
	    set avatar(avatar: Nullable<Avatar>);
	    constructor(root: Transform3D,avatar: Nullable<Avatar>,controller: Nullable<AnimatorContorller>);
	    dispose(writeDefaultValues?: boolean): void;
	    update(): void;
	}

}
declare module 'engine/game/3D/animator/AnimatorError' {
	...
	export const AnimatorErrors: {
	    ...
	    5125: (name: string, call: string) => void;
	};

}
declare module 'engine/game/3D/animator/Interruption' {
	import { Nullable } from 'engine/type';
	import Animator from 'engine/game/3D/animator/Animator';
	import { AnimatorStateTransition } from 'engine/game/3D/animator/AnimatorStateTransition';
	import AnimatorState from 'engine/game/3D/animator/AnimatorState';
	export const EmptyState: AnimatorState;
	export let IsInterruptedByDestination: boolean;
	export function GetInterruptionTransition(this: Animator, layerIndex: number, dt: number): Nullable<AnimatorStateTransition>;
	export function GetStateActiveTransition(this: Animator, state: AnimatorState, transition: Nullable<AnimatorStateTransition>, layerIndex: number, normalizedTime: number, dt: number, orderedInterruption: boolean): Nullable<AnimatorStateTransition>;
	export let TransitionRemainedTime: number;

}
declare module 'engine/game/3D/animator/AnimationEvent' {
	import { DeepImmutable } from 'engine/type';
	import AnimationClip from 'engine/assets/classes/animationClip';
	import { Nullable } from 'engine/editor/assets/srcInterface/animatorcontroller';
	export interface IAnimatorClipInfo {
	    clip: DeepImmutable<AnimationClip>;
	    weight: number;
	}
	export default class AnimationEvent {
	    get animatorClipInfo(): IAnimatorClipInfo;
...
declare module 'engine/game/3D/animator/Blending' {
	import AnimationClip from 'engine/assets/classes/animationClip';
	import { SmartArray } from 'engine/misc/array';
	import { DeepImmutable, Nullable } from 'engine/type';
	import Animator from 'engine/game/3D/animator/Animator';
	import AnimatorState from 'engine/game/3D/animator/AnimatorState';
	export interface IAnimationClipInfo {
	    clip: Nullable<DeepImmutable<AnimationClip>>;
	    length: number;
	    weight: number;
	    speed: number;
	    parentIndex: number;
	...
	export const AnimationClipInfos: SmartArray<IAnimationClipInfo>;
	export const MaxBlendTreeNodeNum = 256;
	export function GenerateBlendingInfo(this: Animator, currentState: AnimatorState): number;
	export function ClearGeneratedBlendingInfo(count: number): void;

}
declare module 'engine/game/3D/animator/Allocator' {
	import { SmartArray } from 'engine/misc/array';
	import { IAnimatorClipInfo } from 'engine/game/3D/animator/AnimationEvent';
	import { IAnimatorControllerLayerInfo, IAnimatorStateInfo, IAnimatorStateTransitionInfo } from 'engine/game/3D/animator/Animator';
	import { IAnimationClipInfo } from 'engine/game/3D/animator/Blending';
	...
	export const AnimatorStateTransitionInfoAlloctor: AnimatorStateTransitionInfoAlloctorImpl; class AnimationClipInfoAlloctorImpl implements IAllocator<IAnimationClipInfo> {
	    alloc(): IAnimationClipInfo;
	    free(t: IAnimationClipInfo): void;
	}
	export const AnimationClipInfoAlloctor: AnimationClipInfoAlloctorImpl; class AnimatorClipInfoAlloctorImpl implements IAllocator<IAnimatorClipInfo> {
	    alloc(): IAnimatorClipInfo;
	    free(t: IAnimatorClipInfo): void;
	}
	export const AnimatorClipInfoAllocator: AnimatorClipInfoAlloctorImpl;
	export interface IAnimatorClipBlendInfo {
	    frameIndex: number;
	    weight: number;
	} class AnimatorClipBlendInfoAlloctorImpl implements IAllocator<IAnimatorClipBlendInfo> {
	    constructor();
	    alloc(): IAnimatorClipBlendInfo;
	    free(t: IAnimatorClipBlendInfo): void;
	}
	export const AnimatorClipBlendInfoAllocator: AnimatorClipBlendInfoAlloctorImpl;
	export {};

}
declare module 'engine/game/physics/types' {
...
declare module 'engine/game/physics/utils' {
...
declare module 'engine/math/booleanVector3' {
...
declare module 'engine/shared/physicsNotFound' {
...
declare module 'engine/game/physics/rigidbody' {
	import Component from 'engine/game/component';
	import { ForceMode, CollisionDetectionMode } from 'engine/game/physics/types';
	import BooleanVector3 from 'engine/math/booleanVector3';
	import Vector3 from 'engine/math/vector3';
	import Quaternion from 'engine/math/quaternion';
	...
	export default class Rigidbody extends Component {
	    ...
	    get nativeComp(): phys3D.DynamicRigidbody | undefined;
	    ...
	    addRelativeTorque(force: Vector3, mode: ForceMode): void;
...
declare module 'engine/assets/classes/physicMaterial' {
	import BaseResource from 'engine/assets/classes/baseResource';
	import { PhysicMaterialResourceDescription } from 'engine/assets/resource';
	import { BaseLoadContext } from 'engine/assets/load-contexts/load-context-definition';
	import { CombineMode } from 'engine/game/physics/types';
	export default class PhysicMaterial extends BaseResource {
	    ...
	    get nativeMaterial(): phys3D.Material | undefined;
	    static DeserializeResource(resourceDescription: PhysicMaterialResourceDescription, loadContext: BaseLoadContext, dependencies: string[]): PhysicMaterial;
	}

}
declare module 'engine/game/physics/bounds' {
	import Vector3, { Vector3_READONLY } from 'engine/math/vector3'; const boundsMap: WeakMap<phys3D.Bounds, Bounds>;
	export { boundsMap };
	export default class Bounds {
	    get center(): Vector3_READONLY | undefined;
	    get extents(): Vector3_READONLY | undefined;
	    get min(): Vector3_READONLY | undefined;
	    get max(): Vector3_READONLY | undefined;
	    get size(): Vector3_READONLY | undefined;
...
declare module 'engine/game/physics/collider' {
	import Rigidbody from 'engine/game/physics/rigidbody';
	import Component from 'engine/game/component';
	import Entity from 'engine/scene/entity';
	import PhysicMaterial from 'engine/assets/classes/physicMaterial';
	import { Nullable } from 'engine/type';
	import Bounds from 'engine/game/physics/bounds';
	import Vector3 from 'engine/math/vector3';
	import { ColliderType } from 'engine/game/physics/types'; const colliderMap: WeakMap<phys3D.Collider, Collider>; const colliderEventNames: string[];
...
declare module 'engine/game/physics/contactPoint' {
	import Collider from 'engine/game/physics/collider';
	import { Vector3_READONLY } from 'engine/math/vector3'; const contactPointMap: WeakMap<phys3D.ContactPoint, ContactPoint>;
	export { contactPointMap };
	export default class ContactPoint {
	    get separation(): number;
	    get normal(): Vector3_READONLY | undefined;
	    get point(): Vector3_READONLY | undefined;
	    get thisCollider(): Collider | undefined;
	    get otherCollider(): Collider | undefined;
	}

}
declare module 'engine/game/physics/collision' {
	import Collider from 'engine/game/physics/collider';
	import ContactPoint from 'engine/game/physics/contactPoint';
	import { Vector3_READONLY } from 'engine/math/vector3'; const collisionMap: WeakMap<phys3D.Collision, Collision>;
	export { collisionMap };
	export default class Collision {
	    get impulse(): Vector3_READONLY | undefined;
	    get relativeVelocity(): Vector3_READONLY | undefined;
	    get collider(): Collider | null | undefined;
	    get contacts(): ContactPoint[] | undefined;
	}
...
declare module 'engine/game/3D/animator/Animator' {
	import type Avatar from 'engine/assets/classes/avatar';
	import Component from 'engine/game/component';
	import type { DeepImmutable, Nullable } from 'engine/type';
	import AnimatorBlendTree from 'engine/game/3D/animator/AnimatorBlendTree';
	import AnimatorCondition from 'engine/game/3D/animator/AnimatorCondition';
	import AnimatorController, { ParamenterChangeEvent } from 'engine/game/3D/animator/AnimatorController';
	import AnimatorControllerLayer from 'engine/game/3D/animator/AnimatorControllerLayer';
	import AnimatorControllerParameter from 'engine/game/3D/animator/AnimatorControllerParameter';
	import AnimatorState from 'engine/game/3D/animator/AnimatorState';
	import { AnimatorStateMachine } from 'engine/game/3D/animator/AnimatorStateMachine';
	import { AnimatorStateTransition } from 'engine/game/3D/animator/AnimatorStateTransition';
	import { ControllerBinding } from 'engine/game/3D/animator/ControllerBinding';
	import type { Entity3D } from 'engine/scene/scene';
	import { SmartArray } from 'engine/misc/array';
	import { Emitter, Listener } from 'engine/misc/emitter';
	import { IAnimatorClipBlendInfo } from 'engine/game/3D/animator/Allocator';
	import type AvatarMask from 'engine/assets/classes/avatarMask';
	import AnimationClip from 'engine/assets/classes/animationClip';
	import AnimationEvent from 'engine/game/3D/animator/AnimationEvent';
	import { MetaAnimatorControllerStateModel } from 'engine/game/3D/animator/MetaAnimation';
	...
	interface IAnimatorControllerLayerInfoTransition {
	    currentState: AnimatorState;
	    nextState: AnimatorState;
	    currentTransition: AnimatorStateTransition;
	    stateInfo: IAnimatorStateInfo;
	    nextStateInfo: IAnimatorStateInfo;
	    transitionInfo: IAnimatorStateTransitionInfo;
	}
	export type IAnimatorControllerLayerInfo = IAnimatorControllerLayerInfoState | IAnimatorControllerLayerInfoTransition;
	export default class Animator extends Component {
	    readonly entity: Entity3D;
	    static readonly AnimationEvent: typeof AnimationEvent;
	    static readonly AnimatorBlendTree: typeof AnimatorBlendTree;
	    static readonly AnimatorCondition: typeof AnimatorCondition;
	    static readonly AnimatorController: typeof AnimatorController;
	    static readonly AnimatorControllerLayer: typeof AnimatorControllerLayer;
	    static readonly AnimatorControllerParameter: typeof AnimatorControllerParameter;
	    static readonly AnimatorState: typeof AnimatorState;
	    static readonly AnimatorStateMachine: typeof AnimatorStateMachine;
	    static readonly AnimatorStateTransition: typeof AnimatorStateTransition;
	    static readonly AnimatorCullingMode: typeof AnimatorCullingMode;
	    static readonly Limits: Readonly<{
	        MaxBlendTreeNodeNum: number;
	    }>;
	    get layers(): Readonly<AnimatorControllerLayer[]>;
	    ...
	    get avatar(): Nullable<Avatar>;
	    set avatar(avatar: Nullable<Avatar>);
	    get controller(): Nullable<AnimatorController>;
	    set controller(controller: Nullable<AnimatorController>);
	    priority: number;
	    readonly rebindEvent: Emitter<Animator>;
	    constructor(entity: Entity3D);
	    ...
	    /**
	     * Play a state
	     * @param {string} stateName The name or hash generated for a state of animator, when name is used layer should be with the string, eg. 'Base Layer.Run'.
	     ...
	     * @param {string} fullOrPartialName The name or hash generated for a state of animator, when name is used layer should be with the string, eg. 'Base Layer.Run'.
	     * @param {float} normalizedTransitionDuration The duration of the transition.
...
declare module 'engine/game/3D/skinnedMeshRenderer' {
	...
	export default class SkinnedMeshRenderer extends MeshRenderer {
	    ...
	    get isActiveInLastFrame(): boolean;
	    constructor(entity: Entity3D);
	}
...
declare module 'engine/game/particles/libs/hermiteCurve' {
	...
	export default class HermiteCurve {
	     ...
	     * @param data data:
	     */
	    static Deserialize(data: ISerializeData, curve: HermiteCurve, context?: {
	        [key: string]: any[];
	    }, builtContext?: {
	        [key: string]: SmartArray<any>;
	    }): HermiteCurve;
...
declare module 'engine/game/3D/trailRenderer' {
	...
	export default class TrailRenderer extends MeshRenderer {
	    ...
	    get lastPosition(): Vector3 | undefined;
	    set lastPosition(val: Vector3 | undefined);
	    get curPosition(): Vector3 | undefined;
	    set curPosition(val: Vector3 | undefined);
...
declare module 'engine/game/particles/libs/boxShape' {
	...
	import { default as ParticleShape, EmitFromBox } from 'engine/game/particles/libs/shape'; type Particle = import('engine/game/particles/particle').default;
	export default class ParticleBoxShape extends ParticleShape {
	    ...
	    static GetEmitPositionAndDirection(dataBuffer: Float32Array, offsetMap: IParticleEmitterShapeOffsetMap, dst: Float32Array, posDirtOffsetMap: {
	        posOffset: number;
	        dirOffset: number;
	    }, randomNum: number): void;
	    /**
	     * 粒子从盒子的哪个位置喷射
	     * @type {EmitFromBox}
	     ...
	     * @memberof ParticleBoxShape
	     */
	    getEmitPosAndDir(dst: Float32Array, posDirtOffsetMap: {
	        posOffset: number;
	        dirOffset: number;
	    }): void;
	}
...
declare module 'engine/game/particles/libs/arcShape' {
	import { IParticleEmitterShapeOffsetMap } from 'engine/game/particles/interface';
	import { ArcMode, ParticleShape } from 'engine/game/particles/libs/shape'; type Particle = import('engine/game/particles/particle').default;
...
declare module 'engine/game/particles/libs/circleShape' {
	import Vector3 from 'engine/math/vector3';
	import { IParticleEmitterShapeOffsetMap } from 'engine/game/particles/interface';
	import ParticleArcShape from 'engine/game/particles/libs/arcShape'; type Particle = import('engine/game/particles/particle').default;
...
declare module 'engine/game/particles/libs/coneShape' {
	...
	import { EmitFromCone } from 'engine/game/particles/libs/shape'; type Particle = import('engine/game/particles/particle').default;
...
declare module 'engine/game/particles/modules/particleEmitterShape' {
	...
	import ParticleModule from 'engine/game/particles/particleModule'; type Particle = import('engine/game/particles/particle').default;
	...
	export default class ParticleEmitterShape extends ParticleModule {
	    static configToArray(config: IParticleEmitterShape, offset: number, bufferData?: number[]): {
	        propertyOffsetMap: IParticleEmitterShapeOffsetMap;
	        curOffset: number;
	    };
	    static CreateByDefaultVal(particle: Particle): ParticleEmitterShape;
	    static GetEmitterShapeData(dataBuffer: Float32Array, offsetMap: IParticleEmitterShapeOffsetMap, dst: Float32Array, randomNum: number, speedOffset: number, positionOffset: number): void;
	    static getUniformFromBuffer(bufferData: Float32Array, offsetMap: IParticleOffsetMap): {
	        key: string;
	        value: number[];
	    };
...
declare module 'engine/game/particles/libs/shape' {
	import Vector3 from 'engine/math/vector3';
	import { IParticleEmitterShapeOffsetMap } from 'engine/game/particles/interface'; type Particle = import('engine/game/particles/particle').default;
	export enum ArcMode {
	    Random = 1,
	    Loop = 2,
	    PingPong = 3,
...
declare module 'engine/game/particles/interface' {
	...
	export type IParticleModulesOffsetMapAll = IParticleCommonOffsetMap & IParticleEmitterOffsetMap & IParticleEmitterBurstsOffsetMap & IParticleRendererOffsetMap & IParticleEmitterShapeOffsetMap & IParticleRotationByLifeOffsetMap & IParticleSizeByLifeOffsetMap & IParticleSpeedByLifeOffsetMap & IParticleSpeedLimitByLifeOffsetMap & IParticleColorByLifeOffsetMap & IParticleTextureSheetAnimationOffsetMap;
...
declare module 'engine/game/particles/bufferManager' {
	...
	import { ConstantProperty, CurveProperty, GradientProperty, OneColorProperty, TwoColorsProperty, TwoConstantsProperty, TwoCurvesProperty, TwoGradientsProperty } from 'engine/game/particles/libs/advancedProperty';
	export function handleAdvancedPropertyToData(config: AdvancedProperty | ColorAdvancedProperty, needUniform?: boolean, needComputed?: boolean): {
	    rawData: number[];
	    computedData: number[];
	    uniformData: number[];
	} | null;
	export function handleVectorPropertyToData(arr: number[], offset: number, bufferData: number[]): false | {
	    offsetMap: IVectorPropertyOffsetMap;
	    curOffset: number;
	};
	export function getAdvancedPropertyComputedValue(defaultBuffer: Float32Array, offsetMap: IParticleModulesOffsetMapOne, key: keyof IParticleModulesOffsetMapAll, isInit?: boolean, randomSeed?: number): number;
	...
	export function getColorAdvancedPropertyUniformValue(defaultBuffer: Float32Array, offsetMap: IParticleModulesOffsetMapOne, key: keyof IParticleModulesOffsetMapAll, dstAlpha1?: Float32Array, dstColor1?: Float32Array, dstAlpha2?: Float32Array, dstColor2?: Float32Array): Float32Array | {
	...
	export function getVectorProperty(defaultBuffer: Float32Array, offsetMap: IParticleModulesOffsetMapOne, key: keyof IParticleModulesOffsetMapAll, dst: Vector2 | Vector3 | Vector4, defaultVal: Vector2 | Vector3 | Vector4): Vector2 | Vector3 | Vector4;
	...
	export function getAdvancedProperty(defaultBuffer: Float32Array, offsetMap: IParticleModulesOffsetMapOne, key: keyof IParticleModulesOffsetMapAll): ConstantProperty | CurveProperty | TwoConstantsProperty | TwoCurvesProperty | undefined;
	export function setAdvancedProperty(offsetMap: IParticleModulesOffsetMapOne, key: keyof IParticleModulesOffsetMapAll, val: ConstantProperty | CurveProperty | TwoConstantsProperty | TwoCurvesProperty | undefined): void;
	export function getColorAdvancedProperty(defaultBuffer: Float32Array, offsetMap: IParticleModulesOffsetMapOne, key: keyof IParticleModulesOffsetMapAll): OneColorProperty | TwoColorsProperty | GradientProperty | TwoGradientsProperty | undefined;
	export function setColorAdvancedProperty(offsetMap: IParticleModulesOffsetMapOne, key: keyof IParticleModulesOffsetMapAll, val: OneColorProperty | TwoColorsProperty | GradientProperty | TwoGradientsProperty | undefined): void;
...
declare module 'engine/game/particles/modules/particleColorByLife' {
	...
	export default class ParticleColorByLife extends ParticleModule {
	    static configToArray(config: IParticleColorByLife, offset: number, bufferData?: number[]): {
...
declare module 'engine/game/particles/modules/particleRenderer' {
	...
	export default class ParticleRenderer extends ParticleModule {
	    static configToArray(config: IParticleRenderer, offset: number, bufferData?: number[]): {
	    ...
	    static setUniformValue(uniformArr: number[] | Float32Array, renderMode: ParticleRenderMode, renderAlignment: ParticleRenderAlignment, lengthScale: number, speedScale?: number, cameraScale?: number): Float32Array | number[];
...
declare module 'engine/game/particles/modules/particleRotationByLife' {
	...
	import ParticleModule from 'engine/game/particles/particleModule'; type Particle = import('engine/game/particles/particle').default; type AdvancedProperty = ConstantProperty | CurveProperty | TwoConstantsProperty | TwoCurvesProperty;
	...
	export default class ParticleRotationByLife extends ParticleModule {
	    static configToArray(config: IParticleRotationByLife, offset: number, bufferData?: number[]): {
...
declare module 'engine/game/particles/modules/particleCommon' {
	import { IParticleCommon, IParticleCommonOffsetMap, IParticleOffsetMap } from 'engine/game/particles/interface';
	import { ConstantProperty, CurveProperty, GradientProperty, OneColorProperty, TwoColorsProperty, TwoConstantsProperty, TwoCurvesProperty, TwoGradientsProperty } from 'engine/game/particles/libs/advancedProperty';
	import ParticleModule from 'engine/game/particles/particleModule'; type Particle = import('engine/game/particles/particle').default;
	export enum ParticleScalingMode {
	    Hierarchy = 0,
	...
	export default class ParticleCommon extends ParticleModule {
	    static configToArray(config: IParticleCommon, offset: number, bufferData?: number[]): {
	        propertyOffsetMap: IParticleCommonOffsetMap;
	        curOffset: number;
	    };
	    /**
	     * 与非静态方法getCommonData功能一样，通过config提前生成静态数据。
	     *
	     * @static
	     * @memberof ParticleCommon
	     */
	    static GetCommonData(dataBuffer: Float32Array, offsetMap: IParticleOffsetMap, dst: Float32Array, dstU32: Uint32Array, randomNum: number, startLifeTimeOffset: number, startSizeOffset: number, startColorOffset: number, startRotationOffset: number, startSpeedOffset: number, isUIParticle: boolean): void;
	    /**
	     * 是否开启XYZ三个方向的大小设置。
	     * @type {boolean}
	     * @memberof ParticleCommon
...
declare module 'engine/game/particles/libs/emitterBurst' {
	import { IParticleEmitterBursts, IParticleEmitterBurstsOffsetMap, IParticleOffsetMap } from 'engine/game/particles/interface';
	import { ConstantProperty, CurveProperty, TwoConstantsProperty, TwoCurvesProperty } from 'engine/game/particles/libs/advancedProperty'; type Particle = import('engine/game/particles/particle').default;
	...
	export default class ParticleEmitterBurst {
	    static configToArray(config: IParticleEmitterBursts, offset: number, bufferData?: number[]): {
	        propertyOffsetMap: any;
	        curOffset: number;
	    };
	    static GetBurstEmitTimeCountList(dataBuffer: Float32Array, offsetMap: IParticleOffsetMap, burstOffsetMap: IParticleEmitterBurstsOffsetMap, dst: Array<{
	        time: number;
	        count: number;
	    }>): void;
	    /**
...
declare module 'engine/game/particles/modules/particleEmitter' {
	import { IParticleEmitter, IParticleEmitterOffsetMap, IParticleOffsetMap } from 'engine/game/particles/interface';
	import { ConstantProperty, CurveProperty, TwoConstantsProperty, TwoCurvesProperty } from 'engine/game/particles/libs/advancedProperty';
	...
	export default class ParticleEmitter extends ParticleModule {
	    static configToArray(config: IParticleEmitter, offset: number, bufferData?: number[]): {
	        propertyOffsetMap: IParticleEmitterOffsetMap;
	        curOffset: number;
	    };
	    static CreateByDefaultVal(particle: Particle): ParticleEmitter;
	    static GetEmitTimeCountList(dataBuffer: Float32Array, offsetMap: IParticleOffsetMap): Array<{
	        time: number;
	        count: number;
	    }>;
	    /**
...
declare module 'engine/game/particles/modules/particleSizeByLife' {
	...
	export default class ParticleSizeByLife extends ParticleModule {
	    static configToArray(config: IParticleSizeByLife, offset: number, bufferData?: number[]): {
...
declare module 'engine/game/particles/modules/particleSpeedByLife' {
	...
	export default class ParticleSpeedByLife extends ParticleModule {
	    static configToArray(config: IParticleSpeedByLife, offset: number, bufferData?: number[]): {
...
declare module 'engine/game/particles/modules/particleSpeedLimitByLife' {
	...
	export default class ParticleSpeedLimitByLife extends ParticleModule {
	    static configToArray(config: IParticleSpeedLimitByLife, offset: number, bufferData?: number[]): {
...
declare module 'engine/game/particles/modules/particleTextureSheetAnimation' {
	...
	export default class ParticleTextureSheetAnimation extends ParticleModule {
	    static configToArray(config: IParticleTextureSheetAnimation, offset: number, bufferData?: number[]): {
...
declare module 'engine/game/particles/particleVertexLayout' {
	import VertexLayout from 'engine/render/vertexLayout'; const particleVertexLayout: VertexLayout;
...
declare module 'engine/game/particles/particle' {
	...
	export default class Particle extends MeshRenderer {
	    ...
	    static CalParticleVertexIndicesBuffer(dataBuffer: Float32Array, offsetMap: IParticleOffsetMap, isUIParticle?: boolean): {
	        vertexBuffer: Float32Array;
	        indicesBuffer: Uint16Array;
	    };
	    static GetActiveParticleCount(emitList: Array<{
	        time: number;
	        count: number;
	    }>, currentTime: number, startLifeTime: number): number;
	    static GetParticleConstantData(dataBuffer: Float32Array, offsetMap: IParticleOffsetMap): {
	        rendererMesh: Mesh | undefined;
	        isMesh: boolean;
	        vertexCount: number;
	        indices: number[];
	        subPositionAndUV: Float32Array[];
	    };
	    static SetSingleParticleVertexBuffer(vDataBufferView: Float32Array, vDataBufferViewU32: Uint32Array, subPositionAndUV: Float32Array[], dataBuffer: Float32Array, offsetMap: IParticleOffsetMap, vertexCount: number, isMesh: boolean, index: number, emitTime: number, // [0, duration]，真正的发射时间需要在shader中算出，需增加一个uniform，startEmitTime
	    isUIParticle: boolean): void;
	    /**
	     * 检测是否可以使用烘培模式
	     * @static
	     * @param {Float32Array} dataBuffer
	     * @param {IParticleOffsetMap} offsetMap
	     * @returns
	     * @memberof Particle
	     */
	    static CanUseBakedMode(dataBuffer: Float32Array, offsetMap: IParticleOffsetMap): boolean;
	    ...
	    startTime: number;
	    alreadySetStartEmitTime: boolean;
	    startLifeTimeMaxVal: number;
	    rebuildBuffer(forceInit?: boolean): void;
	    needForceRebuildBuffer(val: any): void;
	}
...
declare module 'engine/assets/classes/texture2D' {
	import Texture, { TextureFilterMode, TexturePixelFormat, TextureWrapMode } from 'engine/assets/classes/texture'; type TextureSource = HTMLImageElement | HTMLCanvasElement | ArrayBufferView;
	...
	export default class Texture2D extends Texture {
	    ...
	    /**
	     * 使用压缩纹理初始化
	     * 目前支持 PVR/ASTC (IOS) , ETC1/ETC2 (Android) , S3TC(DXT1/DXT5) (Windows)
	     *
...
declare module 'engine/assets/buildin-asset/buildin-texture' {
	export { BuildInTextures } from 'engine/assets/classes/texture2D';
	export { BuildInCubeTextures } from 'engine/assets/classes/textureCube';
...
declare module 'engine/game/baseCamera' {
	...
	export default abstract class BaseCamera extends Component {
	     ...
	     */
	    get layerCullDistances(): Array<number>;
	    set layerCullDistances(v: Array<number>);
	    /**
	     * 投影矩阵
	     *
	     * @readonly
...
declare module 'engine/materials/pass' {
	...
	export interface IPassRenderStates {
	    ...
	    blendSrc: wgfx.EnumBlendFactor;
	    blendDst: wgfx.EnumBlendFactor;
	    blendSrcRGB?: wgfx.EnumBlendFactor;
	    blendSrcAlpha?: wgfx.EnumBlendFactor;
	    blendDstRGB?: wgfx.EnumBlendFactor;
	    blendDstAlpha?: wgfx.EnumBlendFactor;
	    blendFunc: wgfx.EnumBlendOp;
	    cullOn: boolean;
	    cullFace: wgfx.EnumCullMode;
	    depthWrite: boolean;
...
declare module 'engine/render/baseRenderDraw' {
	...
	import UniformBlockValue from 'engine/render/uniformBlockValue';
	import { Emitter } from 'engine/misc/emitter';
	import { ShadowMode } from 'engine/game/3D/camera'; type Material = import('engine/assets/classes/material').default; type Mesh = import('engine/assets/classes/mesh').default; type Renderable2D = import('engine/game/2D/renderable2D').default; type Pass = import('engine/materials/pass').default;
	...
	export default class RenderDrawHelper {
	    ...
	    static getRendererType(rd: MeshRenderer): EnumRendererType;
	     ...
	     * @memberof RenderDrawHelper
	     */
	    static encodeRenderDraw(rd: wxGA.RenderDraw, renderView: wxGA.RenderView, meshRenderer: MeshRenderer, subMeshIndex: number, material: Material, pass: Pass, globalShaderDefination: ShaderDefination, globalUniformValue: UniformBlockValue, globalImages: {
	        [name: string]: wgfx.Image;
	    }, depthPixelFormat: wgfx.EnumPixelFormat, shadowMode: ShadowMode): boolean;
	    static encodeUIRenderDraw(rd: wxGA.RenderDraw, renderView: wxGA.RenderView, render2D: Renderable2D, material: Material, vData: wxGA.BatchableVertexBuffer, iData: wxGA.BatchableIndexBuffer, indiceNum: number, globalShaderDefination: ShaderDefination, globalUniformValue: UniformBlockValue, globalImages: {
	        [name: string]: wgfx.Image;
	    }, depthPixelFormat?: wgfx.EnumPixelFormat): boolean;
	    static encodeSkyBoxProcessRenderDraw(rd: wxGA.RenderDraw, renderView: wxGA.RenderView, lightMode: string, material: Material, globalUniformBlockValue: UniformBlockValue): boolean;
	    static encodePostProcessRenderDraw(rd: wxGA.RenderDraw, renderView: wxGA.RenderView, sourceTex: wgfx.Image, depthTex: wgfx.Image, pass: Pass, finalPass: boolean, globalUniformBlockValue?: UniformBlockValue, customUniformBlockValue?: UniformBlockValue): boolean | undefined;
	    static buildSubMeshRenderDraws(object: MeshRenderer, renderView: wxGA.RenderView, lightMode: string, globalShaderDefination: ShaderDefination, globalUniformValue: UniformBlockValue, globalImages: {
	        [name: string]: wgfx.Image;
	    }, depthPixelFormat?: wgfx.EnumPixelFormat, shadowMode?: ShadowMode): wxGA.RenderDraw[];
	    ...
	    static rebuildUIRenderDrawsMaterial(renderDraws: Array<wxGA.RenderDraw>, renderable2D: Renderable2D, globalShaderDefination: ShaderDefination, globalUniformValue: UniformBlockValue, globalImages: {
	        [name: string]: wgfx.Image;
	    }): boolean;
	}
	export {};
...
declare module 'engine/system/animation' {
	import { MetaAnimatorControllerModel } from 'engine/game/3D/animator/MetaAnimation';
	import { IGameSystem } from 'engine/game/game';
	import { SmartArray } from 'engine/misc/array'; type TransformBase = import('engine/game/transformBase').default; type Transform3D = import('engine/game/transform3D').default; type MetaAnimationComponent = import('engine/game/3D/animator/meta').MetaAnimationComponent;
	export default class AnimationSystem implements IGameSystem {
	    ...
	    markAnimatorControllerNeedUpdate(comp: wxGA.IAnimatorController): void;
	    ...
	    cancelAnimatorControllerUpdate(comp: wxGA.IAnimatorController): void;
...
declare module 'engine/system/node' {
	...
	import TransformBase from 'engine/game/transformBase';
	import { SmartArray } from 'engine/misc/array';
	import { Nullable } from 'engine/type'; type Component = import('engine/game/component').default; type Entity = import('engine/scene/entity').default; enum ComponentState {
	    Entry = 0,
	    ToActivate = 1,
	    Activating = 2,
	    Activated = 3,
...
declare module 'engine/game/2D/label/util' {
	export const split: (text: string) => RegExpMatchArray | null;

}
declare module 'engine/game/open-data/compile' {
	interface Scope {
	    [key: string]: any;
	}
...
declare module 'engine/graphicResource/texturePacker' {
	...
	import Rect from 'engine/math/rect';
	import { Nullable } from 'engine/type';
	import { SmartArray } from 'engine/misc/array'; class BlockTreeNode extends Rect {
	    used: boolean;
...
declare module 'engine/assets/buildin-asset/buildin-context' {
	...
	export default _default; class BuildinContext extends BaseLoadContext {
	    constructor();
	    getKey(): string;
	    abort(): void;
	    registerTask(): void;
...
declare module 'engine/assets/loader' {
	...
	export class Loader {
	     ...
	     */
	    register(url: string, base: string, isBuildIn?: boolean): LitePromise<void>;
	    /**
	     *
	     * @template T 资源类型
...
declare module 'engine/game/2D/UISprite' {
	...
	import { Nullable } from 'engine/type';
	import Renderable2D from 'engine/game/2D/renderable2D';
	import OpenData from 'engine/game/open-data'; enum UISpriteType {
	    Simple = 0,
	    Sliced = 1,
	    Filled = 3
...
declare module 'engine/input/touch' {
	...
	import { Entity2D } from 'engine/scene/scene';
	import { DeepImmutable, Nullable } from 'engine/type';
	import { Listener } from 'engine/misc/emitter'; type Component = import('engine/game/component').default;
...
declare module 'engine/game/2D/input/KeyboardInputComponent' {
	import { Delegate } from 'engine/misc/delegate';
	import Component from 'engine/game/component'; enum KeyboardEventNames {
...
declare module 'engine/game/open-data/safe' {
...
declare module 'engine/game/open-data' {
	import LitePromise from 'lib/utils/promise';
	import SpriteFrame from 'engine/assets/classes/spriteFrame'; type PrimitiveType = number | string | boolean | null | undefined | {
...
declare module 'engine/game/2D/label/UILabel' {
	...
	import TextSystem from 'engine/system/text';
	import { TextAlign, TextVAlign, overflowType } from 'engine/game/2D/label/enum';
	import OpenData from 'engine/game/open-data';
...
declare module 'engine/game/2D/label/glyph/query' {
	...
	import { Glyph } from 'engine/game/2D/label/glyph'; const queryGlyph: (text: string, italic: boolean, bold: boolean, fontSize: number, fontFamily: string) => Glyph; const measureText: (text: string, italic: boolean, bold: boolean, fontSize: number, fontFamily: string, emoji: boolean) => {
	    width: number;
	    height: number;
	};
...
declare module 'engine/system/text' {
	import { SmartArray } from 'engine/misc/array';
	import { IGameSystem } from 'engine/game/game';
	import UILabel from 'engine/game/2D/label/UILabel';
	import OpenData from 'engine/game/open-data';
	export default class TextSystem implements IGameSystem {
	     ...
	     */
	    queryText(text: string | OpenData, italic: boolean, bold: boolean, fontSize: number, fontFamily: string, emoji: boolean): void;
	    /**
	     * 文本预备批量处理队列提交到客户端。
	     */
	    updateTexts(): void;
...
declare module 'engine/worker/worker' {
	 ...
	 */
	import { Nullable } from 'engine/type';
	import SyncPair from 'engine/worker/syncPair'; type StatusEnum = "init" | "running" | "terminated" | "invalid";
...
declare module 'engine/worker/job' {
	...
	import EngineWorker from 'engine/worker/worker'; type StatusEnum = "init" | "starting" | "running" | "suspended" | "destroying" | "destroyed" | "invalid";
	export default class WorkerJob {
	    ...
	    get sync(): SyncPair;
	    constructor(worker: EngineWorker, jobPath: string, data: object, usingName?: boolean);
	    /**
	       * 启动job
...
declare module 'engine/worker/rpc' {
	import EngineWorker from 'engine/worker/worker'; type StatusEnum = "init" | "starting" | "running" | "done" | "invalid";
...
declare module 'engine/worker/module' {
	/**
	 * @file workerModule
	 * @author shanexyzhou
	 */
...
declare module 'engine/system/worker' {
	...
	import { Nullable } from 'engine/type';
	import { IGameSystem } from 'engine/game/game';
	import WorkerModule from 'engine/worker/module'; type Game = import('engine/game/game').default;
	export enum Module {
	    LOGIC = 0,
	    WX_API = 1
	}
	...
	export default class WorkerSystem implements IGameSystem {
	    readonly game: Game;
	    /**
	       * 暂时让两个worker使用同一个入口，没有进行代码分离
	       */
	    logicWorker: Nullable<WorkerModule>;
	    wxWorker: Nullable<WorkerModule>;
	    workers: Array<WorkerModule>;
	    ...
	    /**
	       ...
	       * @param m 模块名
	       */
	    queryModule(m: Module): boolean | null;
	}
	export {};

}
declare module 'engine/game/physics/controllerColliderHit' {
	import Collider from 'engine/game/physics/collider';
	import Vector3 from 'engine/math/vector3';
	import { CharacterController } from 'engine/engine'; const controllerColliderHitsMap: WeakMap<phys3D.ControllerColliderHit, ControllerColliderHit>;
	export { controllerColliderHitsMap };
	export default class ControllerColliderHit {
	    constructor(native: phys3D.ControllerColliderHit);
	    get collider(): Collider | undefined;
	    get controller(): CharacterController | null | undefined;
	    get moveDirection(): Vector3;
	    get normal(): Vector3;
	    get point(): Vector3;
	    get moveLength(): number;
	    get entity(): import('engine/engine').Entity | undefined;
	    get rigidbody(): import('engine/game/physics/rigidbody').default | null | undefined;
	    get transform(): import('engine/game/transform3D').default | undefined;
	}

}
declare module 'engine/game/physics/charactercontroller' {
	import Collider from 'engine/game/physics/collider';
	import { CollisionFlags } from 'engine/game/physics/types';
	import PhysicMaterial from 'engine/assets/classes/physicMaterial';
	import Vector3 from 'engine/math/vector3';
	export default class CharacterController extends Collider {
	    ...
	    set collisionFlags(v: CollisionFlags);
	    get detectCollisions(): boolean;
	    set detectCollisions(v: boolean);
...
declare module 'engine/game/physics/raycasthit' {
	import Vector3 from 'engine/math/vector3';
	export default class RayCastHit {
	    ...
	    /**
	       ...
	       */
	    get rigidbody(): import('engine/type').Nullable<import('engine/game/physics/rigidbody').default>;
	    /**
	       * 被射线射中的collider
	       */
	    get collider(): import('engine/game/physics/collider').default | null;
	    /**
	       * 从光线的原点到碰撞点的距离
	       */
	    get distance(): number;
...
declare module 'engine/system/physics' {
	import Vector3 from 'engine/math/vector3';
	import Component from 'engine/game/component';
	import Rigidbody from 'engine/game/physics/rigidbody';
	import CharacterController from 'engine/game/physics/charactercontroller';
	import Collider from 'engine/game/physics/collider';
	import RaycastHit from 'engine/game/physics/raycasthit';
	import Game, { IGameSystem } from 'engine/game/game';
	import { QueryTriggerInteraction } from 'engine/game/physics/types';
	import Quaternion from 'engine/math/quaternion';
	export default class PhysicsSystem implements IGameSystem {
	    ...
	    /**
	     * 设置物理引擎每一帧是否要自动模拟，默认为true。
	     * 如果设置了自动模拟，每fixedDeltaTime会进行一次模拟物理模拟运算，如果因为某些逻辑卡主较长时间，会在一次物理update调用执行多次物理模拟运算。
	     */
	    autoSimulation: boolean;
	    constructor(game: Game);
	    /**
	     * 射线检测用来检测给定的线段穿过哪些碰撞体，我们还可以获取到碰撞体在线段穿过碰撞体的那个点的法线向量和其他一些有用的信息。
	     * rayCast用来判断射线范围内是否存在任何collider
	     * @param {Vector3} origin 射线起点
	     * @param {Vector3} unitDir 射线方向
	     * @param {Vector3} distance 射线的最大距离
	     * @param {RaycastHit} hit 用来接收射线结果的信息
	     * @param {number} layerMask 用来有选择的忽略某些layer的碰撞体
	     */
	     ...
	     * @param {Vector3} p1 胶囊的起点
	     * @param {Vector3} p2 胶囊的结束点
	     * @param {number} radius 胶囊的半径
	     * @param {Vector3} direction 扫描胶囊体的方向
	     * @param {number} maxDistance 投射的最大距离
	     * @param {number} layerMask 根据Layer mask层的不同来忽略碰撞体
	     * @param {number} queryTriggerInteraction 指定此次查询是否需要触发触发器
	     */
	    ...
	    /**
	     * 对场景中的碰撞体进行胶囊的投射，如果相交，返回碰撞体的raycasthit信息。
	     * @param {Vector3} origin 扫描开始处的球体中心
	     * @param {number} radius 该球体的半径
	     * @param {Vector3} direction 扫描球体的方向
	     * @param {number} maxDistance 投射的最大长度
	    ...
	    /**
	     * 与sphereCast类似，但该函数将返回球体扫描相交的所有命中对象
	     * @param {Vector3} origin 扫描开始处的球体中心
	     * @param {number} radius 该球体的半径
	     * @param {Vector3} direction 扫描球体的方向
	     * @param {number} maxDistance 投射的最大长度
	    ...
	    /**
	     * 沿射线投射盒体并返回有关命中对象的详细信息，与boxCast类似，但返回所有命中对象
	     * @param {Vector3} center  盒体的中心
	     * @param {Vector3} halfExtents 盒体各个维度大小的一半
	     * @param {Vector3} direction 投射盒体的方向
	     * @param {Quaternion} orientation 盒体的旋转
	     ...
	     * 获取与给定盒体重叠的碰撞体
	     * @param {Vector3} center 盒体的中心
	     * @param {Vector3} halfExtends 盒体各个维度大小的一半
	     * @param {Quaternion} orientation 盒体的旋转
	     * @param {number} layermask 层遮罩，用于在投射射线时有选择地忽略碰撞体
	     * @param {numver} queryTriggerInteraction 指定该查询是否应该命中触发器
	     */
	    ...
	    /**
	     * 与胶囊体接触或位于胶囊体内部的碰撞体
	     * @param {Vector3} p1 胶囊体在 start 处的球体中心
	     * @param {Vector3} p2 胶囊体在 end 处的球体中心
	     * @param {number} radius 胶囊体的半径
	     * @param {number} layermask 层遮罩，用于在投射胶囊体时有选择地忽略碰撞体
	     * @param {number} queryTriggerInteraction 指定该查询是否应该命中触发器
	     */
	    ...
	    /**
	     * 切换场景的时候需要重置物理引擎，业务侧无需关心
	     */
	    clear(): void;
	    /**
	     * 物理引擎初始化或者场景切换的时候需要根据配置重置物理全局配置信息，引擎内部调度，业务侧无需关心。
	     */
	    setParam(): void;
	    /**
	     * 原生层真正的物理系统实例，业务侧无需关心。
	     */
	    get nativeSystem(): phys3D.PhysSystem | undefined;
	    update(dt: any): void;
	    /**
	     * 当设置物理引擎每一帧是不需要自动模拟的时候，可以通过手动调用simulate的方式来进行物理模拟运算；
	     * 如果将与帧率相关的步长值传递给物理引擎，则由于可能出现的不可预测的帧率波动，
	     * 要获得确定性的物理结果，每次调用 simulate 时都应该向其传递一个固定的步长值。
	     * @param dt
	     */
...
declare module 'engine/game/3D/pointLight' {
	import Vector3 from 'engine/math/vector3';
	import Component from 'engine/game/component';
	import MeshRenderer from 'engine/game/3D/meshRenderer';
	import { Entity3D } from 'engine/scene/scene';
...
declare module 'engine/system/light' {
	import { IGameSystem } from 'engine/game/game';
	import PointLight from 'engine/game/3D/pointLight';
	import { PriorityArray } from 'engine/system/node'; type Game = import('engine/game/game').default; class InvokeLightOnce<T extends PointLight> extends PriorityArray<T> {
	...
	export default class LightSystem implements IGameSystem {
	    readonly game: Game;
	    lightSet: Set<PointLight>;
	    onEnter(light: PointLight): void;
	    onRemove(light: PointLight): void;
	    constructor(game: Game);
	    update(): void;
	    dispose(): void;
	}
...
declare module 'engine/game/game' {
	...
	import UICanvas from 'engine/game/2D/UICanvas';
	import { Emitter } from 'engine/misc/emitter';
	import { DirectionalLightEventType, MeshRendererEventType, PipeLineEventType } from 'engine/event';
	import WorkerSystem from 'engine/system/worker';
	import PhysicsSystem from 'engine/system/physics';
	import LightSystem from 'engine/system/light';
	...
	export default class Game {
	     ...
	     */
	    createWorkerJob(jobPath: string, data: any): import('engine/type').Nullable<import('engine/engine').WorkerJob>;
	    /**
	     * TODO
	     * in progress
	     */
...
declare module 'engine/system/render' {
	...
	import Screen from 'engine/render/screen';
	import { Nullable } from 'engine/type';
	import { MeshRendererEventType, DirectionalLightEventType } from 'engine/event'; type Game = import('engine/game/game').default;
	/**
	 * @public
	 */
...
declare module 'engine/render/process/forwardBasePassProcess' {
	...
	export default class ForwardBasePassProcess extends RenderProcess {
	     ...
	     */
	    update(cullResult: ITypeCullResult, object2RenderDrawsForwardAdd: Map<MeshRenderer, wxGA.RenderDraw[]>): void;
	    ...
	    clearAllObjectsRenderCache(): void;
	    resetObjectSubMeshInfo(meshRenderer: MeshRenderer): void;
	    setShadowParams(mode: ShadowMode, shadowMap: Nullable<RenderTexture>, csmFarBounds: Nullable<Float32Array>, shadowTilingOffsets: Nullable<Float32Array>, csmLightSpaceMatrices: Nullable<Float32Array>): void;
	}
...
declare module 'engine/render/process/shadowMapPassProcess' {
	...
	import { SmartArray } from 'engine/misc/array';
	import OBB from 'engine/math/OBB';
	export const CSM_MAX_SPLITS = 4; class ShadowSetting {
...
declare module 'engine/game/3D/spotLight' {
	import MeshRenderer from 'engine/game/3D/meshRenderer';
	import { Entity3D } from 'engine/scene/scene';
	import { V3ReadOnly } from 'engine/math/types';
	import PointLight from 'engine/game/3D/pointLight';
...
declare module 'engine/render/process/forwardAddPassProcess' {
	import RenderTexture from 'engine/assets/classes/renderTexture';
	import Camera, { ShadowMode } from 'engine/game/3D/camera';
	import MeshRenderer from 'engine/game/3D/meshRenderer';
	import ShaderDefination from 'engine/materials/shaderDefination';
	...
	import { Nullable } from 'engine/type';
	import UniformBlockValue from 'engine/render/uniformBlockValue';
	import { ITypeCullResultAddPass } from 'engine/render/process/cameraRenderProcess';
	import RenderProcess from 'engine/render/process/renderProcess';
	import { SmartArray } from 'engine/misc/array';
	import PointLight from 'engine/game/3D/pointLight';
	export default class ForwardAddPassProcess extends RenderProcess {
	    get frameObject2RenderDraws(): Map<MeshRenderer, wxGA.RenderDraw[]>;
	    constructor(scene: RenderSystem, camera: Camera, renderView: wxGA.RenderView, defaultObjectCount: number);
	    /**
	     * 每帧更新
	     *
	     ...
	     * @memberof ForwardBasePassProcess
	     */
	    update(cullResult: ITypeCullResultAddPass): void;
	    setRenderView(renderView: wxGA.RenderView): void;
	    clearObjectRenderCache(meshRenderer: MeshRenderer): void;
	    resetObjectSubMeshInfo(meshRenderer: MeshRenderer): void;
	    clearAllObjectsRenderCache(): void;
...
declare module 'engine/render/process/skyboxProcess' {
	...
	export default class SkyboxProcess extends RenderProcess {
	    constructor(scene: RenderSystem, camera: Camera, renderView: wxGA.RenderView);
...
declare module 'engine/render/postProcess/basePostProcess' {
	...
	export default abstract class BasePostProcess {
	     ...
	     * @param commandBuffer
	     */
	    run(commandBuffer: wxGA.CommandBuffer, globalUniformBlock?: UniformBlockValue, final?: boolean): void;
	    destroy(): void;
	}
...
declare module 'engine/render/postProcess/type' {
	 type Bloom = typeof import('engine/render/postProcess/bloom').default; type FXAA = typeof import('engine/render/postProcess/fxaa').default; type HDR = typeof import('engine/render/postProcess/hdr').default; type DepthOfField = typeof import('engine/render/postProcess/dof').default;
	export enum Effects {
...
declare module 'engine/render/process/cameraRenderProcess' {
	...
	export interface ITypeCullResultAddPass {
	    objects: SmartArray<MeshRenderer>;
	    additionalLights: SmartArray<PointLight>;
	}
...
declare module 'engine/game/3D/LODGroup' {
	import Component from 'engine/game/component';
	import { Entity3D } from 'engine/scene/scene';
	import { SmartArray } from 'engine/misc/array';
	import MeshRenderer from 'engine/game/3D/meshRenderer';
...
declare module 'engine/game/3D/camera' {
	...
	import Transform3D from 'engine/game/transform3D';
	import { Listener } from 'engine/misc/emitter';
	import { ILod, ICenter } from 'engine/game/3D/LODGroup';
	import { MeshRenderer } from 'engine/engine';
	...
	export default class Camera extends BaseCamera {
	     ...
	     * @memberof Camera
	     */
	    static lodGroup: Map<number, ILodGroupItem>;
	    static addLodGroupItem(lodGroupItem: ILodGroupItem): void;
	    static removeLodGroupItem(lodGroupId: number): void;
	    /**
	     * 阴影渲染模式
	     *
	     * @type {ShadowMode}
...
declare module 'engine/game/3D/meshRenderer' {
	...
	export default class MeshRenderer extends Component {
	     ...
	     *
	     * @modified by shanexyzhou 2020.08.20
	     * 不再作为固有属性，而是由entity的layer计算出
	     *
	     * @type {number}
	     * @memberof MeshRenderer
	     */
	    ...
	    /**
	     * 当meshRenderer在光源的有效范围内，就把光源加到有效队列中
	     *
	     * @param {PointLight} [light] 待加入列表的局部光源
	     * @memberof MeshRenderer
	     */
	    addAffectLight(light: PointLight): void;
	    get additionalLights(): Array<PointLight>;
	    addAdditionalLightIndex(index: number): void;
	    refreshAdditionalLightsUniform(): void;
	    /**
	     * 将光源移出有效光源队列
	     *
	     * @param {PointLight} [light] 待删除的局部光源
	     * @memberof MeshRenderer
	     */
	    removeAffectLight(light: PointLight): void;
	    /**
	     * 全量更新光源列表
	     *
	     * @memberof MeshRenderer
...
declare module 'engine/game/2D/UICanvas' {
	...
	import { UILayerChangeType, Renderable2DEventType } from 'engine/event';
	import Renderable2D from 'engine/game/2D/renderable2D';
	import { SmartArray } from 'engine/misc/array';
	export default class UICanvas extends MeshRenderer {
	    ...
	    readonly renderable2DEvent: Emitter<Renderable2D, Renderable2DEventType>;
...
declare module 'engine/game/transform2D' {
	...
	export default class Transform2D extends TransformBase {
	     ...
	     * @memberof Transform2D
	     */
	    constructor(entity: Entity2D,rawNativeValue?: Float32Array,raw?: Float32Array,offset?: number);
	    /**
	     * 添加子节点，若子节点已存在父节点，则会先移除其父节点
	     * 子节点必须是Transform2D
	     *
...
declare module 'engine/assets/deserialize/decorators' {
	import Component from 'engine/game/component';
	import Entity from 'engine/scene/entity';
	import { Nullable } from 'engine/type'; type BaseResource = import('engine/assets/classes/baseResource').default; type BaseLoadContext = import('engine/assets/load-contexts/load-context-definition').BaseLoadContext;
...
declare module 'engine/assets/deserialize/deserialize' {
	...
	import { SmartArray } from 'engine/misc/array'; type BaseLoadContext = import('engine/assets/load-contexts/load-context-definition').BaseLoadContext;
...
declare module 'engine/assets/classes/scene' {
	...
	import BaseResource from 'engine/assets/classes/baseResource';
	import { Scene3D, Scene2D } from 'engine/scene/scene';
	/**
	 * 场景资源。
	 * @export
	 * @class Scene
	...
	export default class Scene extends BaseResource {
	     ...
	     * @memberof Scene
	     */
	    instantiate(): Scene3D | Scene2D | null;
...
declare module 'engine/scene/scene' {
	...
	export interface EnvironmentMapData {
	    color: TextureCube;
	} type Material = import('engine/assets/classes/material').default; type Game = import('engine/game/game').default; type DirectionalLight = import('engine/game/3D/directionalLight').default;
	...
	export class Scene3D {
	    ...
	    environmentMaps: EnvironmentMapData[];
...
declare module 'engine/game/3D/animator/MetaAnimation' {
	import { Entity3D } from 'engine/scene/scene';
	import Transform3D from 'engine/game/transform3D';
	import AnimationClip from 'engine/assets/classes/animationClip';
	...
	export class MetaAnimationClipModel {
	    ...
	    get curveCount(): number;
	    constructor(ab: ArrayBuffer, clip: AnimationClip, fps: number);
	...
	export function createSyncFunction(entity: Entity3D, property: string): ISyncFunction;
	...
	export class MetaAnimationClipBinding {
	    ...
	    constructor(clipArray: MetaAnimationClipModel[], clipArrayOffset: number, clipArrayLength: number, entityArray: Array<Transform3D | null>, channelArrayOffset: number, channelArrayLength: number, useDefaultAddedNodesAction: UseDefaultAddedAction);
	    updateBinding(clipArray: MetaAnimationClipModel[], clipArrayOffset: number, clipArrayLength: number, entityArray: Array<Transform3D | null>, channelArrayOffset: number, channelArrayLength: number, removeAction: number, retainedAction: number, addedAction: UseDefaultAddedAction): boolean;
	    writeDefaultValues(): void;
	}
	export function createMetaAnimationClipBinding(clipArray: MetaAnimationClipModel[], clipArrayOffset: number, clipArrayLength: number, entityArray: Array<Transform3D | null>, channelArrayOffset: number, channelArrayLength: number, useDefaultAddedNodesAction: UseDefaultAddedAction): MetaAnimationClipBinding;
	...
	export class MetaAnimatorControllerStateModel {
	    readonly clips: MetaAnimationClipModel[];
	    nextState: MetaAnimatorControllerStateModel | null;
	    weight: number;
	    useDefault: number;
	    blendCount: number;
	    info: IMetaAnimatorContorllerStateInfo[];
	    constructor(clips: MetaAnimationClipModel[], offset: number, blendCount: number);
	    resetBlendInfo(): void;
	    setNextState(state: MetaAnimatorControllerStateModel | null): void;
	    setBlendInfo(clip: MetaAnimationClipModel, frameIndex: number, blendWeight: number, additiveReferenceClip: null | MetaAnimationClipModel, additiveFrameIndex: number): void;
	    getAnimationClipIndex(clip: MetaAnimationClipModel): number;
	}
	export function createMetaAnimatorControllerStateModel(clips: MetaAnimationClipModel[], offset: number, length: number): MetaAnimatorControllerStateModel;
	export enum BlendType {
	    Override = 0,
	    Additive = 1
	}
	...
	export class MetaAnimatorControllerModel {
	    ...
	    setLayerMask(layerIndex: number, mask: null): void;
	    setLayerMask(layerIndex: number, mask: Array<{
	        entity: Entity3D;
	        weight: number;
...
declare module 'engine/assets/classes/animationClip' {
	import { MetaAnimationClipModel } from 'engine/game/3D/animator/MetaAnimation';
	import type { Nullable } from 'engine/type';
...
declare module 'engine/assets/classes/atlas' {
	import { BaseLoadContext } from 'engine/assets/load-contexts/load-context-definition';
	import { AtlasResourceDescription } from 'engine/assets/resource';
	import BaseResource from 'engine/assets/classes/baseResource';
	import SpriteFrame from 'engine/assets/classes/spriteFrame';
	/**
	 * 图集资源
	...
	export default class AtlasResource extends BaseResource {
	    get spriteframes(): {
	        [name: string]: import('engine/type').Nullable<SpriteFrame>;
	    };
	    getSpriteframeByKey(key: string): import('engine/type').Nullable<SpriteFrame>;
	    /**
	     * 从文件内容初始化图集资源。
	     * @param {*} value
...
declare module 'engine/game/3D/animator/Animation' {
	import type { Entity3D } from 'engine/scene/scene';
	import type { DeepImmutable, Nullable } from 'engine/type';
	import Component from 'engine/game/component';
	import AnimationSystem from 'engine/system/animation';
	...
	import type AnimationClip from 'engine/assets/classes/animationClip';
	import type Transform3D from 'engine/game/transform3D';
	import { MetaAnimationClipBinding, MetaAnimationClipModel, MetaAnimatorControllerModel, MetaAnimatorControllerStateModel } from 'engine/game/3D/animator/MetaAnimation';
	import { Emitter, Listener } from 'engine/misc/emitter';
	import { IAnimatorClipBlendInfo } from 'engine/game/3D/animator/Allocator';
...
declare module 'engine/shared/serialize/serializedObject' {
	import Entity from 'engine/scene/entity';
	export const SerializedObjectKey: string;
	export class BaseSerializedObject<T> extends Array {
	    /**
	     * 获取某一个entity/component/资源的SerializedObject，若没有则创建。
	     *
	     * @static
	     * @param {*} target
	     * @returns
	     * @memberof BaseSerializedObject
	     */
	    static GetSerializedObject(target: any): any;
	    /**
	     * 传入一个entity，生成该entity与component以及所有子entity的serializedObject
	     *
	     * @static
	     * @param {Entity} target
	     * @param {boolean} [withComp=true] 是否对entity下的component生成serializedObject
	     * @param {boolean} [recursive=true] 是否对递归生成每个子entity的serializedObject
	     * @returns
	     * @memberof BaseSerializedObject
	     */
	    static GenerateEntitySerializedObject(target: Entity, withComp?: boolean, recursive?: boolean): void;
	    Serialize?: () => {};
	    onSerialized?: () => {};
	    targetObject: T;
	    properties: string[];
	    ...
	    set autoApplyValue(val: boolean);
	    get hasModifiedProperties(): boolean;
	    set hasModifiedProperties(val: boolean);
	    get isAnimated(): boolean;
	    set isAnimated(val: boolean);
	    constructor(target: T);
	    applyModifiedProperties(): void;
	    findProperty(name: string): void;
	}
	export default class SerializedObject<T> extends BaseSerializedObject<T> {
...
declare module 'engine/shared/serialize/classes/baseResource' {
	...
	import SerializedObject from 'engine/shared/serialize/serializedObject';
	export function SerializeBaseResource(this: SerializedObject<BaseResource>, context: any): string | null;
	export {};
...
declare module 'engine/shared/serialize/shared' {
	...
	import SerializedObject from 'engine/shared/serialize/serializedObject';
	export function Serialize(this: void, target: SerializedObject<any> | any, context?: {
	    [key: string]: any;
	}): ISerializedObject | ISerializedObject["data"];

}
declare module 'engine/shared/serialize/classes/entity' {
	import SerializedObject from 'engine/shared/serialize/serializedObject'; type Entity = import('engine/scene/entity').default;
	export function SerializeEntity(this: SerializedObject<Entity>,context: any): object;
	export {};

}
declare module 'engine/shared/serialize/classes/gradientColor' {
	 type GradientColor = import('engine/game/particles/libs/gradientColor').default; type ISerializeData = import('engine/game/particles/libs/gradientColor').ISerializeData;
	import SerializedObject from 'engine/shared/serialize/serializedObject';
	export function SerializeGradientColor(this: SerializedObject<GradientColor>, context?: {
	    [key: string]: any;
	}): ISerializeData;
	export {};
...
declare module 'engine/shared/serialize/classes/hermiteCurve' {
	 type HermiteCurve = import('engine/game/particles/libs/hermiteCurve').default; type ISerializeData = import('engine/game/particles/libs/hermiteCurve').ISerializeData;
	import SerializedObject from 'engine/shared/serialize/serializedObject';
	export function SerializeHermiteCurve(this: SerializedObject<HermiteCurve>, context?: {
	    [key: string]: any;
	}): ISerializeData;
	export {};
...
declare module 'engine/shared/serialize/classes/transform3D' {
	...
	import SerializedObject from 'engine/shared/serialize/serializedObject';
	export function SerializeTransform3D(this: SerializedObject<Transform3D>, context: any): {
	    position: any;
	    scale: any;
	    rotation: undefined;
	};
...
declare module 'engine/shared/serialize/resources/physicMaterial' {
	 type PhysicMaterial = import('engine/assets/classes/physicMaterial').default;
...
declare module 'engine/shared/serialize/resources/animatorController' {
	...
	import { default as RuntimeAnimatorController } from 'engine/game/3D/animator/AnimatorController';
	export function SerializeAnimatorController(this: AnimatorController, instance: RuntimeAnimatorController): {
	    content?: object;
	    meta?: object;
	};
...
declare module 'engine/shared/serialize/classes/meshRenderer' {
	...
	import SerializedObject from 'engine/shared/serialize/serializedObject';
	export function OnSerializedMeshRenderer(this: SerializedObject<MeshRenderer>, compJson: any): void;
	export {};

}
declare module 'engine/shared/serialize/classes/animator' {
	import SerializedObject from 'engine/shared/serialize/serializedObject'; type Animator = import('engine/game/3D/animator/Animator').default;
	export function OnSerializedAnimator(this: SerializedObject<Animator>, compJson: any): void;
	export {};

}
declare module 'engine/shared/serialize/classes/animation' {
	import SerializedObject from 'engine/shared/serialize/serializedObject'; type Animation = import('engine/game/3D/animator/Animation').default;
	export function OnSerializedAnimation(this: SerializedObject<Animation>, compJson: any): void;
	export {};

}
declare module 'engine/shared/serialize/classes/LODGroup' {
	 type LODGroup = import('engine/game/3D/LODGroup').default;
	import SerializedObject from 'engine/shared/serialize/serializedObject';
	export function SerializeLODGroup(this: SerializedObject<LODGroup>, context: any): {
	    active: any;
	    lodCount: any;
	    lodList: any;
	    renderer: any;
...
declare module 'engine/game/2D/UIGraphic' {
	import Renderable2D from 'engine/game/2D/renderable2D'; enum Shape {
	    Rect = 0,
	    Circle = 1
	}
...
declare module 'engine/game/2D/UIRichText' {
	...
	import TextSystem from 'engine/system/text';
	import Font from 'engine/assets/classes/font';
	import { Nullable } from 'engine/type'; enum RenderType {
	    Text = 0,
	    Image = 1,
	    Animation = 2
	}
...
declare module 'engine/game/2D/UIScrollView' {
	...
	import { Listener } from 'engine/misc/emitter';
	import TransformBase from 'engine/game/transformBase';
	import { BoundsChangeEventType, ChildChangeEventType } from 'engine/event'; enum Movement {
	    Horizontal = 0,
	    Vertical = 1
	}
...
declare module 'engine/game/2D/UIToggle' {
	...
	import { Touchable } from 'engine/game/2D/input/TouchInputComponent';
	import Renderable2D from 'engine/game/2D/renderable2D';
	import ToggleGroup from 'engine/game/2D/UIToggleGroup'; enum ToggleState {
	    UnChecked = 0,
	    Checked = 1
	}
...
declare module 'engine/game/2D/UIWidget' {
	...
	import { Listener } from 'engine/misc/emitter';
	import { SmartArray } from 'engine/misc/array';
	import { BoundsChangeEventType } from 'engine/event'; enum Execute {
	    onEnable = 0,
	    onUpdate = 1,
	    onAwake = 2
	}
...
declare module 'engine/game/raycaster' {
	...
	import Entity from 'engine/scene/entity';
	import Camera from 'engine/game/3D/camera';
	import BoundBall from 'engine/math/boundBall'; class Intersect {
	    entity: Entity;
	    distance: number;
...
declare module 'engine/math/gradient' {
	import Color from 'engine/math/color'; const interpolate: (start: Color, end: Color, offset?: number) => Color; class Gradient {
...
declare module 'engine/gizmo/gizmoDraw/uniformBlockPool' {
	import UniformBlockValue from 'engine/render/uniformBlockValue';
	export class UniformBlockPool {
	    static Get(desc: wgfx.ShaderUniformBlockDesc): UniformBlockValue;
	    static Put(block: UniformBlockValue): void;
	}
	export const CameraUniformBlockDesc: wgfx.ShaderUniformBlockDesc;
	export const ColorUniformBlockDesc: wgfx.ShaderUniformBlockDesc;
	export const TextureSizeUniformBlockDesc: wgfx.ShaderUniformBlockDesc;

}
declare module 'engine/gizmo/gizmoDraw/renderDraw' {
	import RenderTexture from 'engine/assets/classes/renderTexture';
	import ShaderDefination from 'engine/materials/shaderDefination';
	import Rect from 'engine/math/rect';
	import { Nullable } from 'engine/type';
	import { Emitter, Listener } from 'engine/misc/emitter'; type Texture2D = import('engine/assets/classes/texture2D').default; type Pass = import('engine/materials/pass').default; type UniformBlockValue = import('engine/render/uniformBlockValue').default; type Mesh = import('engine/assets/classes/mesh').default; type Camera = import('engine/game/3D/camera').default;
	export class GizmoEncoder {
	    get encoder(): wxGA.CommandBuffer;
	    constructor(scalar: number);
	    update(renderDraws: Array<GizmoRenderDraw | GizmoMeshRenderDraw>): void;
	    updateRawRD(renderDraws: wxGA.RenderDraw[]): void;
	    reset(): void;
	    execute(): void;
	}
	export class GizmoRenderDraw {
	    constructor(uniforms: wgfx.ShaderUniformDesc[], renderPass: Pass, vertexBuffer: Uint8Array, indicesBuffer: Uint16Array, layouts: Array<{
	        name: string;
	        format: wgfx.EnumVertexFormat;
	    }>, vertex: string, pixel: string, imageDescriptions: wgfx.ShaderImageDesc[], images: Texture2D[],renderView: GizmoRenderView);
	    get vertexBuffer(): Uint8Array;
	    get indicesBuffer(): Uint16Array;
	    setVertexBuffer(vertexDataList: Float32Array, offet?: number): void;
	    setIndexBuffer(indexDataList: Uint16Array, offet?: number): void;
	    setArray(key: string, value: Float32Array): void;
	    update(): void;
	    get nativeRenderDraw(): wxGA.RenderDraw;
	    get renderTarget(): RenderTexture | undefined;
	    setDepthWriteOn(depthWritOn: boolean): void;
	    setDepthTest(depthTestOn: boolean, depthTestComp?: wgfx.EnumCompareFunc.ALWAYS): void;
	}
	export class GizmoObservableRect {
	    get width(): number;
	    get height(): number;
	    get x(): number;
	    get y(): number;
	    readonly onChange: Emitter<this>;
	    constructor(x: number,y: number,width: number,height: number);
	    set(x: number, y: number, width: number, height: number): void;
	}
	export class GizmoRenderView {
	    constructor(useRenderTarget: boolean,passAction: wgfx.PassAction,rect: GizmoObservableRect);
	    setTargetTexture(target: Nullable<RenderTexture>, replace: boolean): void;
	    setTargetViewport(target: Rect | undefined): void;
	    get targetViewport(): Rect | undefined;
	    get renderView(): wxGA.RenderView;
	    get renderTexture(): RenderTexture | undefined;
	}
	export class GizmoMeshRenderDraw {
	    constructor(mesh: Mesh,camera: Camera, uniforms: wgfx.ShaderUniformDesc[], renderPass: Pass, vertex: string, pixel: string, imageDescriptions: wgfx.ShaderImageDesc[], images: Texture2D[],renderView: GizmoRenderView);
	    setArray(key: string, value: Float32Array): void;
	    update(): void;
	    get nativeRenderDraws(): wxGA.RenderDraw[];
	    get renderTarget(): RenderTexture | undefined;
	}
	export {};

}
declare module 'engine/gizmo/baseGizmo' {
	import Matrix4 from 'engine/math/matrix4';
	import Rect from 'engine/math/rect';
	import { GizmoObservableRect, GizmoRenderDraw, GizmoRenderView } from 'engine/gizmo/gizmoDraw/renderDraw'; type Camera = import('engine/game/3D/camera').default;
	export abstract class BaseGizmo {
	    constructor();
	    getRenderDrawOfCamera(camera: Camera): GizmoRenderDraw | undefined;
...
declare module 'engine/gizmo/shaders/cube/cube.pixel' {
	 const _default: "\nprecision highp float;\nuniform vec4 u_color;\nvarying vec3 v_normal;\n\nuniform vec3 u_lightDir;\n\nvoid main(void) {\n    vec3 nrmN = normalize(v_normal);\n    vec3 nrmL = -normalize(u_lightDir);\n    float result = max(0.0, dot(nrmN, nrmL));\n    gl_FragColor = vec4((u_color * result).rgb, 1.0);\n}";
	export default _default;

}
declare module 'engine/gizmo/shaders/cube/cube.vertex' {
	 const _default: "\nprecision highp float;\nuniform vec4 u_projection[4];\nuniform vec4 u_view[4];\nuniform vec4 u_world[4];\nattribute vec3 a_position;\nattribute vec2 a_texCoord;\nattribute vec3 a_normal;\nvarying vec3 v_normal;\n\nvarying vec2 v_texCoord;\nuniform vec3 u_worldCameraPosition;\nuniform vec4 u_normal[4];\n\nvoid main(void) {\n    vec4 modelPosition = vec4(a_position, 1.0);\n    vec3 objectNormal = a_normal;\n   \n    vec4 worldPosition = vec4(dot(u_world[0], modelPosition), dot(u_world[1], modelPosition), dot(u_world[2], modelPosition), dot(u_world[3], modelPosition));\n    v_texCoord = a_texCoord;\n    vec4 cameraSpacePosition = mat4(u_view[0], u_view[1], u_view[2], u_view[3]) * worldPosition;\n    v_normal = mat3(mat4(u_normal[0], u_normal[1], u_normal[2], u_normal[3])) * objectNormal;\n    gl_Position = mat4(u_projection[0], u_projection[1], u_projection[2], u_projection[3]) * cameraSpacePosition;\n}";
	export default _default;

}
declare module 'engine/gizmo/actions' {
	export const GetClearAction: () => wgfx.PassAction;
	export const GetLoadAction: () => wgfx.PassAction;

}
declare module 'engine/gizmo/cubeGizmo' {
	import { BaseGizmo } from 'engine/gizmo/baseGizmo';
	import Matrix4 from 'engine/math/matrix4';
	import Vector3 from 'engine/math/vector3';
	import { V3ReadOnly } from 'engine/math/types';
	import Rect from 'engine/math/rect'; type Camera = import('engine/game/3D/camera').default;
	export default class CubeGizmo extends BaseGizmo {
	    get center(): V3ReadOnly;
	    set center(val: V3ReadOnly);
	    get size(): V3ReadOnly;
	    set size(val: V3ReadOnly);
...
declare module 'engine/gizmo/shaders/line/line.pixel' {
	 const _default: "\nprecision highp float;\nuniform vec4 u_color;\n\nvoid main(void) {\n  gl_FragColor = u_color;\n}";
	export default _default;

}
declare module 'engine/gizmo/shaders/line/line.vertex' {
	 const _default: "\nprecision highp float;\nattribute vec3 a_position;\nuniform vec4 u_view[4];\nuniform vec4 u_world[4];\nuniform vec4 u_projection[4];\n\nvoid main(void) {\n    vec4 modelPosition = vec4(a_position, 1.0);\n    vec4 worldPosition = vec4(dot(u_world[0], modelPosition), dot(u_world[1], modelPosition), dot(u_world[2], modelPosition), dot(u_world[3], modelPosition));\n    vec4 cameraSpacePosition = mat4(u_view[0], u_view[1], u_view[2], u_view[3]) * worldPosition;\n    gl_Position = mat4(u_projection[0], u_projection[1], u_projection[2], u_projection[3]) * cameraSpacePosition;\n}";
	export default _default;

}
declare module 'engine/gizmo/lineGizmo' {
	import { BaseGizmo } from 'engine/gizmo/baseGizmo';
	import Matrix4 from 'engine/math/matrix4';
	import Vector3 from 'engine/math/vector3';
	import { V3ReadOnly } from 'engine/math/types';
	import Rect from 'engine/math/rect'; type Camera = import('engine/game/3D/camera').default;
	export default class LineGizmo extends BaseGizmo {
...
declare module 'engine/gizmo/shaders/rect/rect.vertex' {
	 const _default: "\nprecision highp float;\nattribute vec3 a_position;\nuniform vec4 u_view[4];\nuniform vec4 u_world[4];\nuniform vec4 u_projection[4];\n\nvoid main(void) {\n    vec4 modelPosition = vec4(a_position, 1.0);\n    vec4 worldPosition = vec4(dot(u_world[0], modelPosition), dot(u_world[1], modelPosition), dot(u_world[2], modelPosition), dot(u_world[3], modelPosition));\n    vec4 cameraSpacePosition = mat4(u_view[0], u_view[1], u_view[2], u_view[3]) * worldPosition;\n    gl_Position = mat4(u_projection[0], u_projection[1], u_projection[2], u_projection[3]) * cameraSpacePosition;\n}";
	export default _default;

}
declare module 'engine/gizmo/shaders/rect/rect.pixel' {
	 const _default: "\nprecision highp float;\nuniform vec4 u_color;\n\nvoid main(void) {\n  gl_FragColor = u_color;\n}";
	export default _default;

}
declare module 'engine/gizmo/rectGizmo' {
	import { BaseGizmo } from 'engine/gizmo/baseGizmo';
	import Matrix4 from 'engine/math/matrix4';
	import Rect from 'engine/math/rect'; type Camera = import('engine/game/3D/camera').default;
	export default class RectGizmo extends BaseGizmo {
	    get x(): number;
	    set x(val: number);
	    get z(): number;
	    set z(val: number);
...
declare module 'engine/gizmo/circleGizmo' {
	import { BaseGizmo } from 'engine/gizmo/baseGizmo';
	import Matrix4 from 'engine/math/matrix4';
	import Vector3 from 'engine/math/vector3';
	import { V3ReadOnly } from 'engine/math/types';
	import Rect from 'engine/math/rect'; type Camera = import('engine/game/3D/camera').default;
	export default class CircleGizmo extends BaseGizmo {
	    get center(): V3ReadOnly;
	    set center(val: V3ReadOnly);
	    get radius(): number;
	    set radius(val: number);
...
declare module 'engine/gizmo/gizmosPool' {
	import { BaseGizmo } from 'engine/gizmo/baseGizmo';
	...
	export class GizmosPool {
	    ...
	    static cylinderGizmosPool: BaseGizmosPool;
	    static circleGizmosPool: BaseGizmosPool;
	}

}
declare module 'engine/gizmo/gizmos' {
	import Camera from 'engine/game/3D/camera';
	import { BaseGizmo } from 'engine/gizmo/baseGizmo';
	import CubeGizmo from 'engine/gizmo/cubeGizmo';
	import LineGizmo from 'engine/gizmo/lineGizmo';
	import RectGizmo from 'engine/gizmo/rectGizmo';
	import CircleGizmo from 'engine/gizmo/circleGizmo';
	import { V3ReadOnly } from 'engine/math/types';
	import { GizmoRenderView } from 'engine/gizmo/gizmoDraw/renderDraw';
	export { CubeGizmo, LineGizmo, RectGizmo, CircleGizmo };
	export class Gizmos {
	    static viewportToEncodeMap: Map<Camera, wxGA.CommandBuffer[]>;
	    ...
	    /**
	     * @description: 清空Gizmos
	     */
	    static clearAllGizmos(): void;
	    /**
	     * @description: 更新每个gizmo的renderDraw
	     */
	    static update(camera: Camera): void;
	    /**
	     * @description: Gizmos中的CommandBuffer返回给renderer做屏上渲染
	     */
	    static GetCameraEncoderMap(): Map<Camera, wxGA.CommandBuffer[]>;
	}

}
declare module 'engine/worker/wx/commandBuffer' {
	/**
	 * WXAPI指令队列
...
declare module 'engine/worker/wx/utils' {
	export function encodeJsString(str: string): ArrayBuffer;
	export function decodeJsString(ab: ArrayBuffer): string;

}
declare module 'engine/worker/wx/downloadFile' {
	export default function downloadFile(object: engineWX.DownloadFileOptions): engineWX.DownloadTask;

}
declare module 'engine/worker/wx' {
	export { default as downloadFile } from 'engine/worker/wx/downloadFile';

}
declare module 'engine/game/physics/boxCollider' {
	import Collider from 'engine/game/physics/collider';
	import Vector3 from 'engine/math/vector3';
...
declare module 'engine/game/physics/sphereCollider' {
	import Collider from 'engine/game/physics/collider';
	import Vector3 from 'engine/math/vector3';
...
declare module 'engine/game/physics/capsuleCollider' {
	import Collider from 'engine/game/physics/collider';
	import { CapsuleColliderDirection } from 'engine/game/physics/types';
	import Vector3 from 'engine/math/vector3';
...
declare module 'engine/game/physics/meshCollider' {
	import Collider from 'engine/game/physics/collider';
	import Mesh from 'engine/assets/classes/mesh';
...
declare module 'engine/engine' {
	...
	import RawResource from 'engine/assets/classes/raw';
	import Atlas from 'engine/assets/classes/atlas';
	import RenderTexture from 'engine/assets/classes/renderTexture';
	...
	import UILayer from 'engine/game/2D/UILayer';
	import Animation from 'engine/game/3D/animator/Animation';
	import Animator from 'engine/game/3D/animator/Animator';
	import AnimationEvent from 'engine/game/3D/animator/AnimationEvent';
	import Camera from 'engine/game/3D/camera';
	import DirectionalLight from 'engine/game/3D/directionalLight';
	import PointLight from 'engine/game/3D/pointLight';
	import SpotLight from 'engine/game/3D/spotLight';
	import LineRenderer from 'engine/game/3D/lineRenderer';
	import MeshRenderer from 'engine/game/3D/meshRenderer';
	import SkinnedMeshRenderer from 'engine/game/3D/skinnedMeshRenderer';
	import TrailRenderer from 'engine/game/3D/trailRenderer';
	import LODGroup from 'engine/game/3D/LODGroup';
	import Component from 'engine/game/component';
	import Game from 'engine/game/game';
	import Particle from 'engine/game/particles/particle';
	...
	import { Gizmos, CubeGizmo, LineGizmo, RectGizmo, CircleGizmo } from 'engine/gizmo/gizmos';
	import { CacheManager } from 'engine/assets/cache';
	import { NumberJSONMap, ComplexJSONMap } from 'engine/assets/lib/util';
	import { MaterialEventType, MeshEventType, BoundsChangeEventType, ChildChangeEventType, DirectionalLightEventType, Renderable2DEventType, MeshRendererEventType, PipeLineEventType } from 'engine/event';
	import LitePromise from 'lib/utils/promise';
	import EngineWorker from 'engine/worker/worker';
	import WorkerJob from 'engine/worker/job';
	import * as wxRemote from 'engine/worker/wx';
	import Rigidbody from 'engine/game/physics/rigidbody';
	import Collider from 'engine/game/physics/collider';
	import BoxCollider from 'engine/game/physics/boxCollider';
	import SphereCollider from 'engine/game/physics/sphereCollider';
	import CapsuleCollider from 'engine/game/physics/capsuleCollider';
	import MeshCollider from 'engine/game/physics/meshCollider';
	import { ForceMode, CombineMode, RigidbodyConstraints, CapsuleColliderDirection } from 'engine/game/physics/types';
	import PhysicMaterial from 'engine/assets/classes/physicMaterial';
	import RaycastHit from 'engine/game/physics/raycasthit';
	import CharacterController from 'engine/game/physics/charactercontroller';
	import OpenData from 'engine/game/open-data'; const version: string; const workerVersion = "1.1"; const createNativeUUMap: () => wxGA.NativeMap<number>; const createNativeSUMap: () => wxGA.NativeMap<string>; const createNativeULUMap: () => wxGA.LongIntToIntMap;
	export { version, workerVersion, Animation, AnimationClip, AnimationEvent, RawResourceCollection, Animator, AnimatorController, Avatar, AvatarMask, BaseResource, Camera, UICamera, Color, LinearGradient, Component, DirectionalLight, PointLight, SpotLight, Debug, Effect, Entity, FrameSystem, FrameSystemMode, Gizmos, CubeGizmo, LineGizmo, RectGizmo, CircleGizmo, Game, GradientColor, HermiteCurve, KeyboardInputComponent, Material, Matrix3, Matrix4, Mesh, MeshRenderer, NativeBufferReader, Particle, Prefab, Quaternion, Raycaster, BoundBall, Rect, RenderTexture, Scene, Script, SerializeHelper, SkinnedMeshRenderer, LineRenderer, TrailRenderer, LODGroup, SpriteFrame, Texture2D, TextureCube, Font, BitmapFont, RawResource, Atlas, TouchInputComponent, Transform2D, Transform3D, TransformBase, PostProcessComponent, BlendType, UIAnchor, UIButton, UICanvas, UIGraphic, UIGrid, UILabel, TextAlign, TextVAlign, TextRenderMode, TypeNames, UIMask, UIRichText, UIScrollView, UISprite, UITextInput, UIToggle, UIToggleGroup, UIWidget, UILayer, Vector2, Vector3, Vector4, BooleanVector3, buildin2DVertexLayout, buildInVertexLayoutFactory3D, decorators, Touchable, Delegate, ComplexJSONMap, NumberJSONMap, createNativeUUMap, createNativeSUMap, createNativeULUMap, MaterialEventType, MeshEventType, BoundsChangeEventType, ChildChangeEventType, DirectionalLightEventType, Renderable2DEventType, MeshRendererEventType, OpenData, PipeLineEventType, EngineWorker as Worker, WorkerJob, Rigidbody, RaycastHit, Collider, BoxCollider, SphereCollider, CapsuleCollider, MeshCollider, ForceMode, CombineMode, RigidbodyConstraints, PhysicMaterial, CapsuleColliderDirection, CharacterController, LitePromise, wxRemote };
...
declare module 'engine/assets/lib/load-from-ide' {
	export function loadContent(filePath: string, fileType: string, iframe: boolean, fetchUseCache?: boolean): Promise<any>;
	export function loadMeta(resourceID: string, iframe: boolean): Promise<{
	    uuid: string;
	    fileType: string;
	    filePath: string;
	    [key: string]: any;
...
declare module 'engine/assets/load-contexts/load-context-ide' {
	...
	export default class LoadContextIDE extends BaseLoadContext {
	    ...
	    constructor(assetid: string, iframe: boolean, fetchUseCatch: boolean);
	    getKey(): any;
	    getResourceDefinitions(): LitePromise<void>;
	    get resourceDefinitions(): IResourceDefinitions | null;
	    get fileIDs(): string[];
...
declare module 'engine/helper/wireframe/effect' {
	 const _default: {
	    name: string;
	    properties: {
	        key: number;
...
declare module 'engine/misc/collisionMatrixUtils' {
	/**
...
declare module 'lib/utils/thunk' {
	 type Callback<T> = (error: Error | null, res: T | null) => any; type ThunkFunction<T> = (callback: Callback<T>) => void;
	...
	declare const engine: (
	  ...
	  export interface Manager {
	    PhysSystem: typeof PhysSystem;
	    Rigidbody: typeof Rigidbody;
	    RawVec3f: typeof RawVec3f;
	    RawQuaternion: typeof RawQuaternion;
	    ...
	    DynamicRigidbody: typeof DynamicRigidbody;
	    StaticRigidbody: typeof StaticRigidbody;
	    Material: typeof Material;
	    PhysMesh: typeof PhysMesh;
	    RaycastHit: typeof RaycastHit;
	    CharacterController: typeof CharacterController;
	    Collider: typeof Collider;

	    bindRigidbodyToEntity: (rigidbody: phys3D.Rigidbody, entity: wxGA.Entity3D, name: string) => void;
	    alignRigidbodyWithEntity: (rigidbody: phys3D.Rigidbody, entity: wxGA.Entity3D) => void;
	    unbindRigidbody: (rigidbody: phys3D.Rigidbody) => void;
	    bindColliderToEntity: (collider: phys3D.Collider, entity: wxGA.Entity3D) => void;
	    unbindCollider: (collider: phys3D.Collider) => void;
	    bindCCToEntity: (cc: phys3D.Collider, entity: wxGA.Entity3D) => void;
	    unbindCCT: (cc: phys3D.CharacterController) => void;
	    recursivelySyncCCBackEntity: (entity: wxGA.Entity3D, dirty: boolean) => void;
	    clear: () => void;

	    // public bindRigidbodyWithJsObject(rigidbody: phys3D.Rigidbody, jsObject: object);
	    // public getJsObject(rigidbody: phys3D.Rigidbody): object;
	  }
	  ...
	  export class PhysSystem {
	    constructor();
	    constructor(config: physicsDebugConfig | null);
	    gravity: RawVec3f;
...
declare module engineWX {
	  ...
	  class WX {
	    ...
	    createSharedArrayBuffer(length: number): SharedArrayBuffer;
	...
	declare namespace wxGA {
	  ...
	  */
	  ...
	  interface IAnimationClipBinding {
	    id: number;
	    updateBinding(
	      clipArray: AnimationClipModel[], clipArrayOffset: number, clipArrayLength: number,
	      entityArray: Array<Entity3D | null>, entityArrayOffset: number, entityArrayLength: number,
	      removeAction: number, retainedAction: number, addedAction: number,
	    ): void;
	    writeDefaultValues(): void;
	  }
	  ...
	  interface IAnimationClipBinding {
	    ...
	    updateBinding(
	      clipArray: AnimationClipModel[], clipArrayOffset: number, clipArrayLength: number,
	      entityArray: Array<null | Entity3D | number>, entityArrayOffset: number, entityArrayLength: number,
	      removeAction: number, retainedAction: number, addedAction: number,
	    ): boolean;
	    writeDefaultValues(): void;
	  }
	  ...
	  interface IAnimatorController {
	    id: number;
	    setAnimationClipBinding(binding: null | IAnimationClipBinding): void;
	    setLayerBlendType(layerIndex: number, blendType: number): void;
	    setLayerWeight(layerIndex: number, weight: number): void;
	    setLayerBlend(layerIndex: number, blend: null | IAnimatorControllerStateModel): void;
	    setLayerMask(layerIndex: number, mask: null): void;
	    setLayerMask(layerIndex: number, mask: Array<{ entity: wxGA.Entity3D; weight: number }>, offset: number, length: number): void;
	    update(): void;
	  }

	  interface IAnimatorControllerStateModel {
	    id: number;
	    setNextState(state: null | IAnimatorControllerStateModel): void;
	    weight: number;
	    useDefault: number;
	    resetBlendInfo(): void;
	    setBlendInfo(clip: AnimationClipModel, frameIndex: number, blendWeight: number, additiveRefrenceClip: null | AnimationClipModel, additiveFrameIndex: number): void;
	    getAnimationClipIndex(clip: AnimationClipModel): number;
	  }
	  ...
	  class LogicAccelerator {
	    ...
	    */
	    ...
	    public createAnimationClipBinding: (
	      clipArray: AnimationClipModel[], clipArrayOffset: number, clipArrayLength: number,
	      entityArray: Array<number | Entity3D | null>, entityArrayOffset: number, entityArrayLength: number,
	      useDefaultAddedNodesAction: number,
	    ) => IAnimationClipBinding;

	    public createAnimatorController: (layerCount: number) => IAnimatorController;
	    public createAnimatorControllerStateModel: (clips: AnimationClipModel[], offset: number, length: number) => IAnimatorControllerStateModel;
	    ...
	    */
	    ...
	    updateAnimatorControllers: (comps: IAnimatorController[], length: number) => void;
	  ...
	  */
	  ...
	  function getPhysics(): phys3D.Manager;
