...
import Collider from "../commons/collider";
import DataCenter from "../commons/dataCenter";
import EventCenter from "../commons/eventCenter";
import D3Bullet from "./d3Bullet";
import D3Player from "./d3Player";
...
export default class D3Enemy extends engine.Script {
  ...
  public onAwake() {
    ...
    this.direction.z = 1;
    this.hurtParticle = this.entity.transform.children[0].findChildByName("Hurt").entity.getComponent(engine.Particle);
  ...
  public removeEnemy() {
    if (this.entity.transform) {
      const parentTransform = this.entity.transform.parent.entity.transform;
